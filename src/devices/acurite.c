...
#define ACURITE_MSGTYPE_ATLAS_NO_LIGHTNING_WINDSPEED_TEMP_HUMIDITY   0x05
#define ACURITE_MSGTYPE_ATLAS_NO_LIGHTNING_WINDSPEED_RAINFALL        0x06
#define ACURITE_MSGTYPE_ATLAS_NO_LIGHTNING_WINDSPEED_UV_LUX          0x07
...
static int acurite_txr_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
	...
	// The Atlas with lightning sensor is 10 bytes long, but without is 8 bytes and looks like a 5-in-1.
	else if ((message_type >= ACURITE_MSGTYPE_ATLAS_WINDSPEED_TEMP_HUMIDITY && message_type <= ACURITE_MSGTYPE_ATLAS_WINDSPEED_UV_LUX) ||
		(message_type >= ACURITE_MSGTYPE_ATLAS_NO_LIGHTNING_WINDSPEED_TEMP_HUMIDITY && message_type <= ACURITE_MSGTYPE_ATLAS_NO_LIGHTNING_WINDSPEED_UV_LUX)) {
            ...
            if (message_type == ACURITE_MSGTYPE_ATLAS_WINDSPEED_TEMP_HUMIDITY || message_type == ACURITE_MSGTYPE_ATLAS_NO_LIGHTNING_WINDSPEED_TEMP_HUMIDITY) {
            ...
            if (message_type == ACURITE_MSGTYPE_ATLAS_WINDSPEED_RAINFALL || message_type == ACURITE_MSGTYPE_ATLAS_NO_LIGHTNING_WINDSPEED_RAINFALL) {
            ...
            if (message_type == ACURITE_MSGTYPE_ATLAS_WINDSPEED_UV_LUX || message_type == ACURITE_MSGTYPE_ATLAS_NO_LIGHTNING_WINDSPEED_UV_LUX) {
	    ...
	    if (browlen == ACURITE_ATLAS_BITLEN / 8) {
		// An Atlas with lightning detector, has 2 more bytes than Atlas without
                int byte8  = (bb[7] & 0x7f);
                int byte9  = (bb[8] & 0x7f);
        ...
        // The Atlas weather sensor without lightning detector is also 8 bytes, but is handled above.
